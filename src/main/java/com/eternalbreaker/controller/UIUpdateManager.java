...
public class UIUpdateManager {
    ...
    private final List<Timeline> uiTimelines = new ArrayList<>();
    ...
    public void pauseTimers() {
        if (timersRunning) {
            timersRunning = false;
            if (totalTimeUpdater != null)
                totalTimeUpdater.pause();
            if (lastRoundTimeUpdater != null)
                lastRoundTimeUpdater.pause();
        }
    }
    ...
    public void showAndHideMessage(String message, int duration) {
        ...
        statusLabel.setLayoutY((double) GameConstants.SCREEN_HEIGHT / 3);
    ...
    private static String getStyle(String message) {
    ...
    public void setupTotalTimeUsedLabel() {
        totalTimeUpdater = new Timeline(new KeyFrame(Duration.millis(100),
                e -> {
                    ...
                    if (gameState.getGameRoundStopwatch() != null && gameState.getGameRoundStopwatch().isNotPaused()) {
    ...
    public void setupLastRoundTimeUsedLabel() {
        lastRoundTimeUpdater = new Timeline(new KeyFrame(Duration.millis(100),
                e -> {
                    ...
                    if (gameState.getBrickResetStopwatch() != null && gameState.getBrickResetStopwatch().isNotPaused()) {
                        ...
                        gameView.getLastRoundTimeLabel()
                                .setText("Last round time: " + String.format("%.1f", timeToDisplay) + " seconds");
                    }
                }));
